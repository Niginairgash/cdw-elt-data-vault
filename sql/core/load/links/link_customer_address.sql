...
select
  MD5(concat(sc.customer_id, '#', sc.address_id)) as link_customer_address_hk,
  MD5(sc.customer_id) as customer_hk, 
  MD5(sc.address_id) as address_hk,
  now() as load_dts,
...
where not exists(
  ...
  where l.link_customer_address_hk = hashkey(concat(sc.customer_id, '#', sc.address_id))
);
